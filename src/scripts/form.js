...
export default class Form {
  ...
  createEvent(popup) {
    document
      .querySelector(".work-offer__button")
    ...
    popup.querySelector(".form-contact__submit").addEventListener(
      "click",
      function (event) {
        event.preventDefault();

        const items = popup.querySelectorAll(".input");
        let $error = popup.querySelector(".form-contact__error");
        $error.textContent = "";
        let err = false;
        ...
        if (items[1].value == "" || !test1) {
          $error.textContent = "Поле email не заполнено или заполнено неверно";
          err = true;
        ...
        if (err === false) {
          this.sendForm(items).catch(err => {console.log(err);});
